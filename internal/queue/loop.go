...
func RunLoop(ctx context.Context, workerID string, handler JobHandler) {
	...
	for {
		...
		// kill switch (transcode_config.enabled) — shared with the enqueuer
		if !transcodeEnabled(ctx) {
...
// transcodeEnabled reads transcode_config.enabled — missing/malformed = true
// (fail-open: a broken settings doc must not silently stop every worker).
func transcodeEnabled(ctx context.Context) bool {
	...
	if err != nil {
		if !errors.Is(err, mongo.ErrNoDocuments) && ctx.Err() == nil {
			log.Printf("⚠️ Read transcode_config failed: %v", err)
		}
		return true
	}
